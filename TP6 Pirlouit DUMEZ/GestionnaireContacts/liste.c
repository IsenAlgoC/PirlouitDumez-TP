...
int DeleteLinkedListElem(LinkedList * list, SingleLinkedListElem * item) {
	...
	if (item == list->tail) { //item est en queue
		SingleLinkedListElem * previous = GetElementAt(list, list->size - 1);  //initialisation d'un pointeur sur l'élément précédent la queue
		previous->next = NULL;
		list->tail = previous;
		list->size--;
		free(item);
	...
	if ((item != list->head) && (item != list->tail) && (list->size > 0)) { //item n'est ni en queue ni en tête
		int i = 0;
		SingleLinkedListElem* previous = list->head ; //initialisation d'un pointeur NULL sur l'élément précédent
		while ((previous->next != item) && (previous != list->tail )) { //tant qu'on a pas trouve item dans la liste ou qu'on est pas arrivé au bout de la liste
			previous = previous->next;   //on passe a l'élément suivant
		}
		previous->next = item->next; //l'élement suivant le précédent est maintenant l'élément suivant l'item qu'on supprime
		free(item);                  //on libère en mémoire l'espace alloué pour item
		list->size--;
		return 1;
	}
